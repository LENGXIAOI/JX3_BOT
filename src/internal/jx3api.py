...
class JX3APICS:
    ...
    def __new__(cls, *args, **kwargs):
        """单例"""
        if not hasattr(cls, "_instance"):
            orig = super(JX3APICS, cls)
            cls._instance = orig.__new__(cls, *args, **kwargs)
        return cls._instance

    def __init__(self):
        self.config = jx3api_config
        if not self.config.api_urlcs.endswith("/"):
            self.config.api_urlcs += "/"
        token = self.config.api_token or ""
        headers = {"token": token, "User-Agent": "Nonebot2-jx3-bot"}
        self.client = AsyncClient(headers=headers)
    ...
    def __getattr__(self, name: str) -> _ApiCall:
        # 拼接url
        logger.debug(f"<y>jx3api请求功能:</y> | {name}")
        url = self.config.api_urlcs + name.replace("_", "/")
        return partial(self.call_api, url)

class JX3APICSS:
    """
    jx3api接口类，负责访问网站烟花接口，获取数据。
    """
    ...
    def __init__(self):
        ...
        if not self.config.api_url.endswith("/"):
            self.config.api_url += "/"
        token = self.config.api_tokenyh or ""
        headers = {"token": token, "User-Agent": "Nonebot2-jx3-bot"}
        self.client = AsyncClient(headers=headers)
...
class JX3APIJYH:
    ...
    def __init__(self):
        ...
        if not self.config.api_url.endswith("/"):
            self.config.api_url += "/"
        token = self.config.api_tokenjyh or ""
        headers = {"token": token, "User-Agent": "Nonebot2-jx3-bot"}
        self.client = AsyncClient(headers=headers)
...
class JX3APIV8:
    ...
    def __new__(cls, *args, **kwargs):
        """单例"""
        if not hasattr(cls, "_instance"):
            orig = super(JX3APIV8, cls)
            cls._instance = orig.__new__(cls, *args, **kwargs)
        return cls._instance

    def __init__(self):
        self.config = jx3api_config
        if not self.config.api_urlcss.endswith("/"):
            self.config.api_urlcss += "/"
        token = self.config.api_token or ""
        headers = {"token": token, "User-Agent": "Nonebot2-jx3-bot"}
        self.client = AsyncClient(headers=headers)
    ...
    def __getattr__(self, name: str) -> _ApiCall:
        # 拼接url
        logger.debug(f"<y>jx3api请求功能:</y> | {name}")
        url = self.config.api_urlcss + name.replace("_", "/")
        return partial(self.call_api, url)