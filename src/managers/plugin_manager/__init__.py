...
def get_group_setting() -> GroupSetting:
    ...
    def dependency(matcher: Matcher, regex_dict: dict = RegexDict()) -> GroupSetting:
        match regex_dict["value"]:
            ...
            case "八卦推送":
                return GroupSetting.八卦推送
            case "更新推送":
                return GroupSetting.更新推送
            case "诛恶事件":
                return GroupSetting.诛恶事件
            case "关隘首领":
                return GroupSetting.关隘首领
            case "云从社":
                return GroupSetting.云从社
            case "奇遇推送":
...
# 群设置
group_status = group_matcher_group.on_regex(pattern=regex, priority=9, block=False)
# 插件设置
plugin_status = group_matcher_group.on_regex(pattern=regex, priority=10)
