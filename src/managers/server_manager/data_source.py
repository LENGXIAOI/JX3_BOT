...
async def get_ws_status(group_id: int, event: Event.RecvEvent) -> bool:
    ...
    if isinstance(event, Event.VersionEvent):
        recv_type = GroupSetting.更新推送
    if isinstance(event, Event.ZhueshijianEvent):
        recv_type = GroupSetting.诛恶事件
    if isinstance(event, Event.yuncongsheEvent):
        recv_type = GroupSetting.云从社
    if isinstance(event, Event.lankeshanEvent):
        recv_type = GroupSetting.关隘首领
    if isinstance(event, Event.TiebaEvent):
        recv_type = GroupSetting.八卦推送
