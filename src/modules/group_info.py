...
class GroupInfo(Model):
    ...
    """ws-新闻推送开关"""
    ws_VersionEvent = fields.BooleanField(default=True)
    """ws-更新推送开关"""
    ws_TiebaEvent = fields.BooleanField(default=True)
    """ws-八卦推送开关"""
    ws_ZhueshijianEvent = fields.BooleanField(default=False)
    """ws-诛恶事件开关"""
    ws_yuncongsheEvent = fields.BooleanField(default=False)
    """ws-云从社推送开关"""
    ws_lankeshanEvent = fields.BooleanField(default=False)
    """ws-关隘首领推送开关"""
    ws_serendipity = fields.BooleanField(default=False)
    """ws-奇遇推送开关"""
    ws_horse = fields.BooleanField(default=False)
    """ws-抓马推送开关"""
    ws_fuyao = fields.BooleanField(default=False)
    """ws-扶摇推送开关"""
    ...
    async def get_config_status(cls, group_id: int, setting_type: GroupSetting) -> bool:
        ...
        match setting_type:
            ...
            case GroupSetting.更新推送:
                status = record.ws_VersionEvent
            case GroupSetting.诛恶事件:
                status = record.ws_ZhueshijianEvent
            case GroupSetting.云从社:
                status = record.ws_yuncongsheEvent
            case GroupSetting.八卦推送:
                status = record.ws_TiebaEvent
            ...
            case GroupSetting.关隘首领:
                status = record.ws_lankeshanEvent
        return status
    ...
    async def set_config_status(
        ...
        match setting_type:
            ...
            case GroupSetting.更新推送:
                record.ws_VersionEvent = status
            case GroupSetting.诛恶事件:
                record.ws_ZhueshijianEvent = status
            case GroupSetting.云从社:
                record.ws_yuncongsheEvent = status
            case GroupSetting.八卦推送:
                record.ws_TiebaEvent = status
            case GroupSetting.奇遇推送:
            ...
            case GroupSetting.关隘首领:
                record.ws_lankeshanEvent = status
            case _:
                return False
        await record.save()
        return True
    ...
    async def get_meau_data(cls, group_id: int) -> dict:
        ...
        返回:
            * `dict`：菜单数据字典
                ...
                * `ws_ZhueshijianEvent` `bool`：ws诛恶事件推送开关
                * `ws_yuncongsheEvent` `bool`：ws云从社推送开关
                * `ws_TiebaEvent` `bool`：ws八卦推送开关
                * `ws_VersionEvent` `bool`：ws更新推送开关
                * `ws_serendipity` `bool`：ws奇遇推送开关
                * `ws_horse` `bool`：ws抓马推送开关
                * `ws_fuyao` `bool`：ws扶摇推送开关
                * `ws_fulankeshanEvent` `bool`：ws关隘首领推送开关
        ...
        return {
            ...
            "ws_yuncongsheEvent": record.ws_yuncongsheEvent,
            "ws_TiebaEvent": record.ws_TiebaEvent,
            "ws_ZhueshijianEvent": record.ws_ZhueshijianEvent,
            "ws_VersionEvent": record.ws_VersionEvent,
            "ws_serendipity": record.ws_serendipity,
            "ws_horse": record.ws_horse,
            "ws_fuyao": record.ws_fuyao,
            "ws_lankeshanEvent": record.ws_lankeshanEvent,
        }
